...
def _registered_ref_text(registry: dict, ref_audio: str) -> str | None:
    """Return the transcript pinned to this exact registered reference path."""
    try:
        requested = pathlib.Path(ref_audio).resolve()
    except (OSError, RuntimeError):
        return None
    for row in registry.values():
        if not isinstance(row, dict):
            continue
        registered_audio = row.get("ref_audio")
        registered_text = row.get("ref_text")
        if not registered_audio or not isinstance(registered_text, str) or not registered_text.strip():
            continue
        try:
            if pathlib.Path(registered_audio).resolve() == requested:
...
class SpeechReq(BaseModel):
    ...
    num_step: int = 32
    speed: float = 1.0
    language: Literal["zh"] | None = None  # optional target language; omitted preserves existing behavior
...
def speech(req: SpeechReq):
    ...
    # Inline clone: the explicit path chooses the reference. If that exact path is registered
    # with a pinned transcript, carry the pin unless the caller explicitly supplied one.
    # Unregistered candidate refs still bypass registry metadata unchanged.
    if req.ref_audio:
        ...
        ref_text = req.ref_text or _registered_ref_text(reg, req.ref_audio)
